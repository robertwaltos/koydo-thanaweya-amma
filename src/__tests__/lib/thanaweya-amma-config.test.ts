...
import { EXAM_CONFIG } from "@/lib/thanaweya-amma/config";

describe("Thanaweya Amma Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM125");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("thanaweya-amma");
  });
